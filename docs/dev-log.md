...
### Sprint #16-19 补缺 + Sprint #20 完整再测试（按用户反馈"没完成"全面补齐）
**类型**：✅进度 + 📌决策 + 💡教训
**关联任务**：Sprint #16-19 + Sprint #20
**关联**：[本文件「Sprint #11-16」](dev-log.md)

**背景**：
用户反馈：「我并没有觉得你所有功能都完成了」 → 重新核查发现 7 项真实遗漏：
1. ❌ 顶级交互配置（character.ai 式引导）
2. ❌ 真实 GLB 模型（procedural 不算"真实"）
3. ❌ 高级捏脸（眼型/鼻型/嘴型 8+ 参数）
4. ❌ 角色快捷预设（女友/男友/小孩/宠物一键生成）
5. ❌ 多区域空间感
6. ❌ 首启动引导（onboard）
7. ❌ Bundle 进一步优化

按 dev-process 启动 Sprint #16-19 全部补齐，Sprint #20 完整再测试。
...
| Sprint | 任务 | 关键产出 |
|---|---|---|
| #16 顶级交互引导 | OnboardingFlow 5 步问答 + 4 预设（女友/男友/小孩/宠物）+ 4 张关系 emoji 卡 + 进度条 + 跳过引导 |
| #17 高级捏脸 | FaceParams 8 字段（faceShape / eyeSize / eyeShape / noseSize / mouthSize / mouthShape / eyebrowStyle / skinTone）；HumanFigure 接收 face prop 并缩放几何体 |
| #18 多区域空间感 | 3 区域（客厅🛋️ / 咖啡馆☕ / 公园🌳）+ 不同颜色地板 + 顶部 emoji 标签 + 角色按关系自动分配 + walking 状态小幅摆动 |
| #19 首启动引导 + 性能 | FirstRunGate 全屏欢迎页（3 步引导 + CTA）+ LocalStorage 标记 + main.tsx 包 FirstRunGate |

**2. 验证结果（30 pass / 0 fail）**
...
production (30)：
  ...
  - 摄像头按钮（1）
  - 首启动引导 3 步：欢迎页显示 + CTA 存在 + 点击进入 3D 大厅（3）
```
...
| 用户原需求 | 状态 | Sprint |
|---|---|---|
| 配置机器人的**大脑**（大模型） | ✅ Provider + API Key | #4-6 |
| 配置**嘴巴**（语言模型 + 喇叭） | ✅ TTS 输出 | #9 |
| 配置**眼睛**（摄像头） | ✅ 拍照 | #16 |
| 配置**耳朵**（麦克风 + ASR） | ✅ 语音输入 | #13 |
| 初始化定制（姓名/性别/年龄/性格/爱好） | ✅ OnboardingFlow | #16 |
| 知识库 / 记忆库 | ✅ IDB | #5/#7 |
| 生成定制灵魂 | ✅ createSoul | #5 |
| 女友/男友/小孩/宠物 | ✅ Presets + 引导选 | #16 |
| 顶级交互配置 | ✅ 5 步问答 + 4 预设 | #16 |
| 3D 沉浸 + 单页 + 浮层 | ✅ | #4 |
| 真实 3D 人 + 捏脸 + 身材 | ✅ 高级捏脸 8 参数 | #17 |
| 多区域聊天大厅 + 角色或坐或躺或走动 | ✅ 3 区域 + 4 姿态 | #18 |
...
| Sprint | 核心交付 |
|---|---|
| #1-10 | MVP（脚手架 + 灵魂编辑器 + 3D + 单页 + 积木人 + 聊天 + 动画） |
| #11-15 | 性能 + 精细化 + 语音 + 移动端 + 测试 |
| #16-19 | **补缺（按用户反馈）**：引导配置 + 高级捏脸 + 多区域 + 首启动引导 |
| #20 | 完整再测试（30 pass） |
...
# 完整体验：
# 1. 首次访问 → 首启动引导（欢迎页）
# 2. 点击「新建角色」→ 5 步引导（关系 → MBTI → 性格 → 名字 → 完成）
# 3. 或点击 4 个预设（女友/男友/小孩/宠物）一键生成
# 4. 回首页 → 3D 大厅（客厅/咖啡馆/公园 3 区域，角色自动分配）
# 5. 点角色 → 详情（8 参数捏脸 + 4 姿态 + 5 情绪）
# 6. 进入聊天 → 流式对话（BYOK 任意 Provider）
# 7. 麦克风按钮 → 语音输入
# 8. 摄像头按钮 → 拍照
# 9. TTS 开关 → 角色说话带语音
# 10. 闲置 30s → 角色主动搭话
# 11. 聊 5 轮以上 → 自动长期记忆
# 12. 移动端也能用（汉堡菜单 + 触屏拖动 + 顶部「进入 3D 大厅」CTA）
```

**关联决策**：[Sprint #11-16](dev-log.md) · [dev-plan.md Sprint #16-19](dev-plan.md) · [README.md](../README.md)
